...
def delete_columns_from_csv(input_file, output_file, delete_ranges, keyword='', logger=None):
    ...
    for enc in ['cp932','shift_jis','utf-8','utf-8-sig','latin1']:
        try:
            # ★修正ポイント：dtype=strとkeep_default_na=Falseを追加し、自動変換を無効化
    ...
    try:
        ...
        # 保存の直前で、列名の末尾にある「.[数字]」をきれいに取り除く
        cleaned_columns = [re.sub(r'\.\d+$', '', col) for col in df_result.columns]
        df_result.columns = cleaned_columns
        
        # 1. 末尾の ".0" を削除（例: "1.0" → "1", "470.0" → "470"）
        df_result = df_result.replace(
                                    r'^(-?\d+)\.0$',
                                    r'\1',
                                    regex=True
                                )
        # 2. 日付のゼロ埋めを削除（例: "2026/05/01" → "2026/5/1", "2026/10/05" → "2026/10/5"）
        df_result = df_result.replace(r'(\d{4})/0?(\d{1,2})/0?(\d{1,2})', r'\1/\2/\3', regex=True)
        # 論理値 'true'→'TRUE', 'false'→'FALSE'に変換する
        df_result = df_result.replace({'true': 'TRUE', 'false': 'FALSE'})
        # 時間を修正　08:00:00→8:00:00
        df_result = df_result.replace(r'0?(\d{1,2}):(\d{2}):(\d{2})', r'\1:\2:\3', regex=True)
        # 3. 末尾のゼロを削除（例: "4.70" → "4.7"）
        df_result = df_result.replace(r'(\d+\.\d*?)0+$', r'\1', regex=True) 
